<%@ page language="java" contentType="text/html; charset=UTF-8" import="bean.Student,java.util.List"
    pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Insert title here</title>
</head>
<link rel="stylesheet"href="/lesson_choose/bootstrap.min.css">  
<script src="/lesson_choose/jquery.min.js"></script>
<script src="/lesson_choose/bootstrap.min.js"></script>
<style type="text/css">
table.hovertable {
	font-family: verdana,arial,sans-serif;
	font-size:11px;
	color:#333333;
	border-width: 1px;
	border-color: #999999;
	border-collapse: collapse;
}
table.hovertable th {
	background-color:#c3dde0;
	border-width: 1px;
	padding: 8px;
	border-style: solid;
	border-color: #a9c6c9;
}
table.hovertable tr {
	background-color:#d4e3e5;
}
table.hovertable td {
	border-width: 1px;
	padding: 8px;
	border-style: solid;
	border-color: #a9c6c9;
	font-size:15px; 
}
</style>
<script>
function _submit()
{
	var a = document.getElementById("page").value;
	//alert(a);
	//js中null和“”好像不是一个，，具体以后实验
	if(a!="")
		document.getElementById("dz").submit;
	else
		alert("输入不能为空！！！");
	
}
function all_yes()
{
	var most = document.getElementById("most");
	var a = document.getElementsByName("ck");
	if(most.checked == true)
	for(i = 0;i<a.length;i++)
		a[i].checked = true;
	else
		for(i = 0;i<a.length;i++)
			a[i].checked = false;
}
</script>
<body>
<center>
<form action="m_delete_check.action" method=post>
<table class="hovertable">
<tr><th>学号</th><th>姓名</th><th>性别</th><th>学院</th><th>专业</th><th>班级</th><th>操作</th><th><input type="checkbox" id=most onclick="all_yes()"></th>
</tr>
<%List<Student> list = (List<Student>)session.getAttribute("student");
for(Student s : list){ %>
<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
	<td><%=s.getId() %></td><td><%=s.getName() %></td>
	<td><%if(s.getSex()==null)	out.print("未完成"); else out.print(s.getName());%></td>
		<td><%if(s.getXy()==null)out.print("未完成");else out.print(s.getXy()); %></td>
		<td><%if(s.getZy()==null)out.print("未完成");else out.print(s.getZy()); %></td>
		<td><%if(s.getBj()==null)out.print("未完成");else
			out.print(s.getBj());%></td>
			<td><a href="stu_delete.action?id=<%=s.getId()%>">删除</a></td>
			<td><input type="checkbox" name="ck" value=<%=s.getId() %>></td>
</tr>
<%} %>
</table>
<br><input type="submit" value="删除选中项">
</form>
<div>
<ul class="pager">
<%int nowpage = (Integer)session.getAttribute("stu_page");if(nowpage!=1&&nowpage!=0) {%>
    <li class = "previous"><a href="stu_man.action?page=<%=nowpage-1%>">上一页</a></li>
    <%}else
    	{%><li class="previous disabled"><a href="#">上一页</a></li>
    	<%} %>
    	当前为第<%=nowpage %>页，共<%=(Integer)session.getAttribute("stu_endpage")%>页
<%if(nowpage == (Integer)session.getAttribute("stu_endpage")) {%>
<li class="next disabled"><a href="#">下一页</a></li>
<%} else{%>
    <li class = "next"><a href="stu_man.action?page=<%=nowpage+1%>">下一页</a></li>
    <%} %>
</ul>
</div>
<form id="dz" action="stu_man.action" method="post">
<input type=text id="page" name="page">
<input type="button" value="跳转" onclick="_submit()">
</form>
</center>

</body>
</html>